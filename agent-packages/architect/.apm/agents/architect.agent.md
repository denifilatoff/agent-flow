...
`artifactKind`. Use `succeeded` for a completed assessment, `needs-human` for a stage question, and `failed` with an
`error` for a technical failure. In human-input mode, also include `humanGate` with the cited `sourceCommentId`, mapped
