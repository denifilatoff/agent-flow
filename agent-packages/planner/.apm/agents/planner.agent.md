...
`verdict`. Always set `notes` to an array of one or more nonempty strings, never to a string. For a `question` or `unclear`
verdict, publish and receipt exactly one marked question
