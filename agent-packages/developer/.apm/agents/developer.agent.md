...
question and does not authorize a new change request.
...
`summary`, and `artifacts`. A successful stage-mode development result has one artifact containing exactly
`kind: "change-request"`, `number`, `url`, `headSha`, and `state`, using provider-returned values and the 40-character
head SHA. Read the change request back and bind these fields to its actual result after the final push. Never report a
local or stale SHA. Every question or diagnostic comment artifact contains exactly `kind: "comment"`, `id`, `url`,
`marker`, and `artifactKind`, using provider-returned values and the exact marker.
