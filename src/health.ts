...
export function createHealthServer(address: string, port: number, status: OperationalStatus): Server {
  ...
  async function route(request: import("node:http").IncomingMessage, response: import("node:http").ServerResponse) {
    ...
    const session = sessionPath(pathname);
    if (session) {
...
function sessionPath(pathname: string): { flowUuid: string; attemptUuid: string; file: string } | null {
  ...
  try {
    return { flowUuid: decodeURIComponent(parts[3]!), attemptUuid: decodeURIComponent(parts[4]!), file: decodeURIComponent(parts[5]!) };
  } catch {
    return { flowUuid: "", attemptUuid: "", file: "" };
  }
}
