...
const schemaIds: Record<SchemaKind, string> = {
  Stack: "https://agent-flow.dev/schemas/v1/stack.schema.json",
  RuntimeConfig: "https://agent-flow.dev/schemas/v1/runtime-config.schema.json",
...
function compileValidators(schemas: JsonSchema[]): Record<SchemaKind, ValidateFunction> {
  ...
  return Object.fromEntries(Object.entries(schemaIds).flatMap(([kind, id]) => {
    const validator = ajv.getSchema(id);
    return validator ? [[kind, validator]] : [];
...
function validateWith<T>(validators: Record<SchemaKind, ValidateFunction>, kind: SchemaKind, value: unknown): T {
  ...
  if (!validator) throw new Error(`${kind} schema is not loaded`);
  if (!validator(value)) {
...
export async function createDocumentValidator(
  ...
  files: readonly string[] = schemaFiles,
): Promise<DocumentValidator> {
  const canonicalDirectory = await realpath(schemaDirectory);
  const schemas: JsonSchema[] = [];
  for (const file of files) {
    if (!schemaFiles.includes(file as (typeof schemaFiles)[number])) {
      throw new Error(`pinned schema ${file} is not supported`);
    }
